...
import (
	...
	"github.com/ejfkdev/udf/diskkit/util/bitmap"
...
//
// Note, however, that it is much easier to do this using the higher-level APIs at github.com/ejfkdev/udf/diskkit
...
func (fs *FileSystem) OpenFile(p string, flag int) (filesystem.File, error) {
	...
	extents, err := inode.dataBlocks(fs)
	if err != nil {
		return nil, fmt.Errorf("could not read blocks for inode %d: %v", inodeNumber, err)
	}
	return &File{
		inode:       inode,
...
func (fs *FileSystem) readInode(inodeNumber uint32) (*inode, error) {
	...
	if inode.fileType == fileTypeSymbolicLink && inode.linkTarget == "" {
		...
		extents, err := inode.dataBlocks(fs)
		if err != nil {
			return nil, fmt.Errorf("could not read blocks for symlink inode %d: %v", inodeNumber, err)
		}
...
// dataBlocks returns the ordered disk blocks holding this inode's data,
// resolving either the extent tree (extent-based files) or the classic
// i_block pointers (single/double/triple indirect) for non-extent files.
func (in *inode) dataBlocks(fs *FileSystem) (extents, error) {
	if in.flags.usesExtents {
		if in.extents == nil {
			return nil, nil
		}
		return in.extents.blocks(fs)
	}
	return in.indirectBlocks(fs)
}

// indirectBlocks resolves the 15 i_block pointers of a classic (non-extent)
// inode into a flat, ordered extent list, following single/double/triple
// indirect blocks. A zero pointer marks the end of the allocated blocks.
func (in *inode) indirectBlocks(fs *FileSystem) (extents, error) {
	blockSize := uint64(fs.superblock.blockSize)
	pointersPerBlock := int(blockSize / 4)

	readIndirect := func(block uint32) ([]uint32, error) {
		raw := make([]byte, blockSize)
		if _, err := fs.backend.ReadAt(raw, int64(block)*int64(blockSize)); err != nil {
			return nil, err
		}
	...
	emit := func(b uint32) {
		...
		out = append(out, extent{fileBlock: fileBlock, startingBlock: uint64(b), count: 1})
		fileBlock++
	}
	emitAll := func(ptrs []uint32) {
		for _, b := range ptrs {
			if b == 0 {
				return
			}
	...
	for i := 0; i < 12; i++ {
		...
		emit(in.blockPointers[i])
	}
	// Single indirect (i_block[12]).
	if in.blockPointers[12] == 0 {
		return out, nil
	}
	if ptrs, err := readIndirect(in.blockPointers[12]); err == nil {
		emitAll(ptrs)
	} else {
		return nil, err
	}
	// Double indirect (i_block[13]).
	if in.blockPointers[13] == 0 {
		return out, nil
	}
	dbl, err := readIndirect(in.blockPointers[13])
	if err != nil {
		return nil, err
	}
	for _, sb := range dbl {
		...
		if ptrs, err := readIndirect(sb); err == nil {
			emitAll(ptrs)
		} else {
			return nil, err
		}
	...
	if in.blockPointers[14] == 0 {
		return out, nil
	}
	tri, err := readIndirect(in.blockPointers[14])
	if err != nil {
		return nil, err
	}
	for _, db := range tri {
